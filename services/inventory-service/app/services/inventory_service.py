...
def update_stock(db: Session, update_data: InventoryUpdate) -> models.Inventory:
    ...
    item = db.query(models.Inventory).filter(
        models.Inventory.product_id == update_data.product_id
    ...
    if not item:
        if update_data.change_quantity < 0:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Hết hàng")
        
        # Tạo mới
        item = models.Inventory(
            product_id=update_data.product_id,
            quantity=update_data.change_quantity
        )
        db.add(item)
    else:
        # Nếu đã có, cập nhật
    ...
    if item.quantity < 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Không đủ hàng trong kho")
