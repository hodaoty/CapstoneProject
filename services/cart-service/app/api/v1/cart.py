...
def update_cart_item(user_id: str, item: CartItemCreate, db: redis.Redis = Depends(get_redis_db)):
...
def remove_cart_item(user_id: str, product_id: int, db: redis.Redis = Depends(get_redis_db)):
